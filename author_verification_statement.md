...
output compared against the table that the manuscript was drafted from. Per-animal
...
Result: **0 of 9 cell types reach BH p<0.05 at animal level** (smallest BH = 0.341,
...
Both series report 20,684 cells, and per-sample barcode counts correspond 1:1
(the deposited barcode lists are shipped in this repository so the correspondence
can be checked directly). The manuscript therefore treats the field as having one
such experiment, not two.

### 7. References
All six entries were verified against PubMed or the publisher DOI. Five corrections
were required and are recorded in the manuscript's verification note: [1] is
*Anesth Analg* 2018;127(2):496-505, [2] is *Glia* 2013;61(1):71-90, [3] is *Ann
Neurol* 2011;70(6):986-995, [4] author list and DOI corrected, and [6] was replaced
with Lian H *et al.*, *Neuron* 2015;85(1):101-115, the paper that actually supports
the astrocyte-derived-C3 claim. Result: **6/6 resolve to a real record that supports
...
explicitly. All other datasets are public GEO deposits retrieved over HTTPS on
14 September 2026.

### 9. Self-reported counts
Text length (~3,450 words excluding abstract and references), figure count (4 plus
1 supplementary) and reference count (6) were recomputed from the manuscript source,
not carried over.

## Repository and persistent identifier
The analysis pipeline, all intermediate tables, the cached GEO records and matrices,
the limma-voom fits and the figure-generation scripts are assembled as a
version-controlled bundle under the MIT licence and are public at
https://github.com/yyx-4113/pocd-hippocampal-power-audit (branch `main`, release
`v1.0.0`). Large re-downloadable objects (`GSE267933_RAW.tar`, `.h5ad` objects) are
excluded and regenerated by the scripts; the retrieval route is documented in
`README.md`. A persistent Zenodo DOI will be added to this file and to the
