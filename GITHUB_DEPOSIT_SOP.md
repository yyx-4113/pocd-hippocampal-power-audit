...
| 项目 | 状态 |
|---|---|
| 复现包组装 | 已完成：209 个文件，77.7 MB，含全部脚本、中间表、图与 GEO 缓存 |
| **GitHub 仓库** | **已创建、已推送**：<https://github.com/yyx-4113/pocd-hippocampal-power-audit>（Public，默认分支 `main`，commit `1767fd7`） |
| **v1.0.0 发布页** | **已自动生成**：<https://github.com/yyx-4113/pocd-hippocampal-power-audit/releases/tag/v1.0.0>（Actions 打 tag 建 Release，运行成功） |
...
原本有 3 件事只能你本人做。现在**只剩 1 件**：Zenodo 拿永久编号（其余两步已完成）。

---

# ~~第一步：新建仓库~~（已完成，请跳过）

> **状态：已完成。** 仓库已由你创建，代码已推送，`v1.0.0` 标签已打，Release 已自动生成。
> 本节内容保留仅作记录，**不需要再操作**。
...
**5. 建好后跟我说一声** —— **这一步已经走完了**。仓库建好当天，209 个文件已推上 `main`，
`v1.0.0` 标签已打，发布页由 Actions 自动生成（运行状态：成功）。
...
# 第二步：在 Zenodo 上获取永久编号（约 3 分钟）★ 现在只剩这一步
...
# ~~第三步：把默认分支确认为 main~~（已完成，请跳过）

> **状态：已完成。** 仓库默认分支就是 `main`（已通过 GitHub API 核对），无需操作。
